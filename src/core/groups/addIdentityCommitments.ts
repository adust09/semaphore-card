...
export default async function addIdentityCommitments(
  ...
  const rootHashes = await Promise.all(
    names.map((name, i) =>
      previewNewRoot(provider, name, identityCommitments[i])
    )
  );

  //Update the contract with the new root hashes
  const contratAddress = getContractAddress(ContractName.INTERREP_GROUPS);
  const contractInstance = await getBackendContractInstance(
    ContractName.INTERREP_GROUPS,
    contratAddress
  );

  await contractInstance.batchAddRootHashe(
  ...
  await Promise.all(
    names.map((name, i) => appendLeaf(provider, name, identityCommitments[i]))
  );
}
