...
import {
  getPoapGroupNamesByAddress,
  PoapGroupName,
} from "src/core/groups/poap";
import { Web3Provider } from "src/types/groups";
import { dbConnect } from "src/utils/backend/database";
import logger from "src/utils/backend/logger";

export default async function addPoapIdentityCommitmentController(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  try {
    if (
      ethers.utils.verifyMessage(identityCommitment, userSignature) !==
      userAddress
    ) {
    ...
    logger.silly(
      `Adding identity commitment ${identityCommitment} to the tree of the POAP group ${name}`
    );

    await addIdentityCommitment(
      Web3Provider.POAP,
      name as PoapGroupName,
      identityCommitment
    );
