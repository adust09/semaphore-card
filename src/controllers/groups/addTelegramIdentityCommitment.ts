...
export default async function addTelegramIdentityCommitmentController(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  try {
    if (telegramUser.joined) {
      throw new Error(`Telegram user already joind this group`);
    }

    await dbConnect();

    logger.silly(
      `Adding identity commitment ${telegramUserId} to the tree of the Telegram group ${name}`
    );
