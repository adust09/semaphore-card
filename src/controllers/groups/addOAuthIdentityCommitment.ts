...
export default async function addOAuthIdentityCommitment(
  req: NextApiRequest,
  res: NextApiResponse,
    ...
    try {
      switch (provider) {
        case OAuthProvider.GITHUB: {
          const { id, plan, followers, receivedStars } =
        ...
        case OAuthProvider.TWITTER: {
          const { id_str, screen_name, followers_count, verified } =
          ...
          reputation = calculateReputation(OAuthProvider.REDDIT, {
            ...
            botometerOverallScore:
              botometerResult?.display_scores?.universal?.overall,
          });
      ...
      let account = await OAuthAccount.findByProviderAccountId(
        provider,
        accountId
      );
      ...
      switch (name) {
        ...
        default:
          await addIdentityCommitments(
            provider,
            [ReputationLevel.NOT_SUFFICIENT],
            identityCommitment
          );
          break;
      }
  ...
  try {
    ...
    logger.silly(
      `Adding identity commitment ${identityCommitment} to the tree of the ${provider} group ${name}`
    );
